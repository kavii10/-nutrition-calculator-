...
# --- Your existing code loading datasets, defining utility functions, etc. ---
...
def nutrition_api():
    ...
    if not recipe_name:
        return jsonify({"error": "Provide `recipe` query parameter."}), 400
    ...
    if match is not None and not match.empty:
        ...
        translated_nutrition = {translate_nutrient_name(k, lang_code): round(v, 2) for k, v in total_nutrition.items()}
        return jsonify({"recipe": row[lang_col], "nutrition": translated_nutrition, "language": lang_code})
